    ...
    public function getDistributionBeneficiaries(CriteriaGroup $criteriaGroup, Project $project)
    {
        ...
        foreach ($criteriaGroup->getCriteria() as $index => $criterion) {
            ...
            if ($forHousehold && $isCSO) {
                $this->getHouseholdWithCountrySpecificCriterion(
                    $qb,
                    $criterion->getField(),
                    $condition,
                    $criterion,
        ...
        switch ($field) {
            case SelectionCriteriaField::HAS_NOT_BEEN_IN_DISTRIBUTIONS_SINCE:
                $qb->leftJoin('b.assistanceBeneficiary', "db$i")
                    ...
                    ->addSelect(
                        "(CASE WHEN d$i.dateDistribution < :parameter$i THEN d$i.dateDistribution WHEN SIZE(b.assistanceBeneficiary) = 0 THEN :noDistribution ELSE :null END) AS {$criterion->getField()}$i"
                    )
