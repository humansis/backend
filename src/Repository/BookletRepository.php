 ...
 */
class BookletRepository extends EntityRepository
{
    ...
    public function getAllBy($countryIso3, $begin, $pageSize, $sort, $filters = [])
    {
        ...
        if (array_key_exists("sort", $sort) && array_key_exists("direction", $sort)) {
            ...
            } elseif ($value == "numberVouchers") { // If the field is the quantity of vouchers, we sort it by the direction sent
                $q->addGroupBy("b.numberVouchers")->addOrderBy("b.numberVouchers", $direction);
            } elseif ($value == "value") { // If the field is the individual value, we sort it by the direction sent
                $qb->addOrderBy('SUM(v.value)');
            } elseif ($value == "currency") { // If the field is the currency, we sort it by the direction sent
                $q->addGroupBy("b.currency")->addOrderBy("b.currency", $direction);
            } elseif ($value == "status") { // If the field is the status, we sort it by the direction sent
                $q->addGroupBy("b.status")->addOrderBy("b.status", $direction);
            } elseif ($value == "beneficiary") { // If the field is the beneficiaries, we sort it by the direction sent
            ...
            } elseif ($value == "distribution") { // If the field is the distributions, we sort it by the direction sent
