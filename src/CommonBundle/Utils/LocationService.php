...
class LocationService
{
    ...
    */
     ...
     * @param \Closure|null $postImportClosure
     */
    public function importADMFile(string $file, ?int $limit = null, ?\Closure $postImportClosure = null)
    {
        ...
        while ($xml->read()) {
            ...
            if ('adm1' === $xml->name) {
                $adm1 = $this->buildAdm1($name, $code, $iso3);
            } elseif ('adm2' === $xml->name) {
                $adm2 = $this->buildAdm2($name, $code, $adm1);
            } elseif ('adm3' === $xml->name) {
                $adm3 = $this->buildAdm3($name, $code, $adm2);
            } elseif ('adm4' === $xml->name) {
                $this->buildAdm4($name, $code, $adm3);
            }

            if (null !== $postImportClosure) {
                $postImportClosure();
            }
