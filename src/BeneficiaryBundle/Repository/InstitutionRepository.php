    ...
    */
    ...
    public function foundSimilarAddressLevenshtein(string $iso3, string $stringToSearch, int $minimumTolerance)
    {
        ...
        $q = $qb->leftJoin("inst.beneficiaries", "b")
            ...
            // ->leftJoin("inst.institutionLocations", "hl")
    ...
    public function getAllBy($iso3, $begin, $pageSize, $sort, $filters = [])
    {
        ...
        $q = $qb->leftJoin("inst.beneficiaries", "b")
            ...
            ->leftJoin("inst.beneficiaries", "head")
        ...
        if (array_key_exists("sort", $sort) && array_key_exists("direction", $sort)) {
            ...
            if ($value == "currentInstitutionLocation") {
            ...
            elseif ($value == "nationalId") {
                ...
                $q->addOrderBy("inst.id", $direction);
            }

            $q->addGroupBy("inst.id");
        }
        ...
        if (count($filters) > 0) {
            ...
            foreach ($filters as $indexFilter => $filter) {
                ...
                elseif ($category === "livelihood" && count($filterValues) > 0) {
                    ...
                    foreach ($filterValues as $indexValue => $filterValue) {
                        $q->setParameter("filter" . $indexFilter . $indexValue, $filterValue);
                        $orStatement->add($q->expr()->eq("inst.livelihood", ":filter" . $indexFilter . $indexValue));
                    }
                    $q->andWhere($orStatement);
    ...
    public function getAllByIds(array $ids)
    {
        $qb = $this
            ...
            ->leftJoin('inst.institutionLocations', 'location')
    ...
    public function getByHeadAndLocation(
        ...
        $qb = $this->createQueryBuilder('inst')
            ...
            ->innerJoin('inst.institutionLocations', 'hl')
        ...
        if ($locationType === InstitutionLocation::LOCATION_TYPE_CAMP) {
            $qb
                ->leftJoin('hl.campAddress', 'ca')
    ...
    public function configurationQueryBuilder($onlyCount, $countryISO3, Project $project = null)
    {
        $qb = $this->createQueryBuilder("inst");
        if ($onlyCount) {
            $qb->select("count(inst)");
        }

        if (null !== $project) {
            $qb->where(":idProject MEMBER OF inst.projects")
                ->setParameter("idProject", $project->getId());
        }
        $qb->leftJoin("inst.beneficiaries", "b");
    ...
    /**
     * Create sub request. The main request while found institution inside the subrequest (and others subrequest)
     * The institution must have at least one beneficiary with the condition respected ($field $operator $value / Example: gender = 0)
     *
    ...
    {
        $qbSub = $this->createQueryBuilder("inst$i");
        $this->setCountry($qbSub, $countryISO3, $i);
        $qbSub->leftJoin("inst$i.beneficiaries", "b$i")
        ...
        $qb->andWhere($qb->expr()->in("inst", $qbSub->getDQL()))
            ->setParameter("val$i", $filters["value_string"]);
        if (null !== $filters["target"]) {
    ...
    /**
     * Create sub request. The main request while found institution inside the subrequest (and others subrequest)
     * The institution must respect the value of the country specific ($idCountrySpecific), depends on operator and value
     *
     * @param QueryBuilder $qb
     * @param $i
     * @param $countryISO3
    ...
    protected function whereVulnerabilityCriterion(QueryBuilder &$qb, $i, $countryISO3, array $filters)
    {
        ...
        if (boolval($filters["condition_string"])) {
            ...
            $qbSubNotIn = $this->createQueryBuilder("instb$i");
            $this->setCountry($qbSubNotIn, $countryISO3, "b$i");
            $qbSubNotIn->leftJoin("instb$i.beneficiaries", "bb$i")
                ->leftJoin("bb$i.vulnerabilityCriteria", "vcb$i")
                ->andWhere("vcb$i.id = :idvc$i")
                ->setParameter("idvc$i", $filters["id_field"]);

            $qbSub->andWhere($qbSub->expr()->notIn("inst$i", $qbSubNotIn->getDQL()));
        }
    ...
    protected function whereCountrySpecific(QueryBuilder &$qb, $i, $countryISO3, array $filters)
    {
        $qbSub = $this->createQueryBuilder("inst$i");
        $this->setCountry($qbSub, $countryISO3, $i);
        $qbSub->leftJoin("inst$i.countrySpecificAnswers", "csa$i")
    ...
    {
        $qb->leftJoin("inst.institutionLocations", "hl")
            ->leftJoin("hl.campAddress", "ca")
            ->leftJoin("ca.camp", "c")
            ->leftJoin("hl.address", "ad")
    ...
    /**
     * Create sub request to get the country of the institution
     *
     * @param QueryBuilder $qb
     */
    public function getInstitutionCountry(QueryBuilder &$qb)
    {
        $this->getInstitutionLocation($qb);
        $locationRepository = $this->getEntityManager()->getRepository(Location::class);
