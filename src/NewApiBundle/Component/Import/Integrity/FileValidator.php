...
use NewApiBundle\Component\Import\Entity\File as ImportFile;
...
class FileValidator
{
    ...
    public function validate(ImportFile $importFile): void
    {
        ...
        try {
            ...
            $violation = new FileViolation('Unable to read provided file. File is malformed or it has unsupported format.');
        ...
        if ( ($addressViolation = $this->validateAddressColumns($fileHeaders)) instanceof FileViolation) {
    ...
    private function validateMandatoryColumns(array $fileHeaders): array
    {
        ...
        foreach (self::MANDATORY_COLUMNS as $mandatoryColumn) {
            if (!in_array($mandatoryColumn, $fileHeaders)) {
                $violations[] = new FileViolation('This column is mandatory.', [$mandatoryColumn]);
            }
        }
    ...
    private function validateAddressColumns(array $fileHeaders): ?FileViolation
    {
        ...
        if (!($hasCamp || $hasAddress)) {
            return new FileViolation('Missing columns for address or camp', array_merge($addressColumns, $campColumns));
        }
