...
class SmartcardControllerTest extends BMSServiceTestCase
{
    ...
    //
    //     $assistance = null;
    //     foreach ($this->em->getRepository(\DistributionBundle\Entity\Assistance::class)->findAll() as $a) {
    //         foreach ($a->getCommodities() as $commodity) {
    ...
    //         'value' => 255.25,
    //         'distributionId' => $assistance->getId(),
    //         'createdAt' => '2020-02-02T12:00:00Z',
    //     ]);
    //
    //     $smartcard = json_decode($this->client->getResponse()->getContent(), true);
    ...
    //     $distributionBeneficiary = null;
    //     foreach ($this->em->getRepository(\DistributionBundle\Entity\Assistance::class)->findAll() as $a) {
    //         foreach ($a->getCommodities() as $commodity) {
    //             if ('Smartcard' === $commodity->getModalityType()->getName() && count($a->getDistributionBeneficiaries()) > 0) {
    //                 $distributionBeneficiary = $a->getDistributionBeneficiaries()->toArray()[0];
    //                 break;
    //             }
    //         }
    //     }
