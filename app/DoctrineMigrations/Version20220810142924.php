...
final class Version20220810142924 extends AbstractMigration
{
    public function up(Schema $schema): void
    {
        ...
        $this->addSql(
            'UPDATE `household` hh INNER JOIN `abstract_beneficiary` ab ON ab.id = hh.id INNER JOIN `abstract_beneficiary_project` abp ON ab.id = abp.abstract_beneficiary_id INNER JOIN `project` p ON p.id = abp.project_id SET hh.iso3 = p.iso3'
        );
        $this->addSql(
            'UPDATE `household` hh
